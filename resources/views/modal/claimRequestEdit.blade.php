	...
	<form action="{{ url('update-claim-request/') }}/{{$request->id}}" method="POST" id="updateClaimRequest" >
	 @csrf
		...
		<div class="form-group row">
			...
			<label class="col-lg-9 col-xl-8 col-form-label">{{$user->email}}</label>
		</div>
		...
		<div class="form-group row">
			...
			<label class="col-lg-9 col-xl-8 col-form-label">{{viewDateFormat($user->locking_period_start)}}  to  {{viewDateFormat($user->locking_period_end)}}</label>
		</div>
		...
		<div class="form-row mt-4">
			<div class="col-md-12">
				<input id ="claim_id" class="form-check-input" type="hidden" value="{{$request->id}}">
